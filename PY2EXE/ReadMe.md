...
<br>
<h3 align="center">Отчёт по практическому исследованию:<br>Создание запускного файла<br>из Пайтон-скрипта с вкоючением всех ресурсов. <br> по дисциплине "Программирование"</h3>
<br><br>
...
<p align="center"><h1>Создание запускного файла<br>из Пайтон-скрипта с вкоючением всех ресурсов,<br>используемых в Python приложении (графика, звук, шрифты и т.п.)<h1></p>

<h2>Pyinstaller и зачем это всё нужно?</h2>
<h4>
Pyinstaller - специальное приложение, которое входит в состав стандартных библиотек Python и может быть установлено при помощи pip:<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;Однако в некоторых ситуациях такой вариант достаточно удобен по нескольким причинам:<br>
...
<li>можно продемонстрировать работу своей программы, НЕ передавая самого её кода и дополнительных файлов (графики, например)</li>
<li>все файлы пакета собраны в одном файле (компактно, исключена "потеря" отдельных файлов ресурсов)</li>
<li>приложение выглядит как законченный продукт</li>
</ul>
...
<h2>Pyinstaller предлагает доработать процесс сборки напильником или в чём проблема и как с ней бороться?</h2>
&nbsp;&nbsp;&nbsp;&nbsp;Собственно сама сборка пакета представляет собой обычное присоединение к EXE (в НЕ архивированной форме) всех ресурсов. 
...
названием по маске _MEI******, где *-это случайная цифра и просто копирует туда всё содержимое присоединённых к EXE ресурсов, включая и сам
скрипт. И там же и запускает его. Вроде бы всё хорошо, но не всё так просто.
<br>
<h2>Сборка. Описание проблемы добавления файлов в EXE</h2>
&nbsp;&nbsp;&nbsp;&nbsp;При неправильном использовании сборка осуществляется командой: pyinstaller name.py.<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;Использование опции pyinstaller: --onefile или -F (собрать всё в один файл) тоже не решает проблему.
...
&nbsp;&nbsp;&nbsp;&nbsp;Чтобы обойти эту проблему необходимо выполнить добавление файлов сборки ВРУЧНУЮ в файл спецификации сборки *.spec
Для этого выполняем следующие действия:

<ul>
<li>запускаем отдельное приложение, которое ставится при установке pyinstaller: <b>pyi-makespec options name.py</b></li>
<li>в качестве options указываем <b>-F</b> для консольных приложений или -Fw чтобы скрыть появление консоли</li>
...
Каждый файл идет в виде кортежа (tuple) с двумя элементами: 1) относительный (от дирректории) путь к файлу и его название и
название папки, в которую он будет распакован из EXE. Возможны странные варианты, когда мы берем файл из одной папки,
распаковываем его внутри временной _MEI****** папке в другую и там же его используем (не знаю, зачем это может потребоваться, но такая
возможность есть.<br>
...
Запустившемуся PY-скрипту не по силам самомтоятельно найти эти ресурсы и необходимо вносить изменения в код, чтобы обойти эту проблему.<br>
...
<br>
&nbsp;&nbsp;&nbsp;&nbsp;Там же идёт речь, что полный абсолютный путь к папке распаковки сохраняется pyinstaller-ом в системной переменной sys._MEIPASS.
...
<h2>Многова-то получилось.</h2><br>
...
&nbsp;&nbsp;&nbsp;&nbsp;При формировании интерпретатора пакета Python подтягивает большое количество ненужных для работы нашего скрипта библиотек.<br>
...
```
<br>
Инсталлируем все эти библиотеки заново в новое окружение:<br>
<b>.\ven\Scripts\pip.exe install pillow<b><br>
<b>.\ven\Scripts\pip.exe install pygame<b><br>
<b>.\ven\Scripts\pip.exe install pyinstaller<b><br>


Запускаем:<br>
<b>pyi-makespec -F -w -i .\icon.ico name.py</b><br>
Вносим изменения в name.spec-файл.<br>

<b>.\ven\Scripts\pyinstaller.exe name.spec</b><br>
И в появившейся папке dist забираем готовый name.exe<br>
<br>
<b>&nbsp;&nbsp;&nbsp;&nbsp;Вот теперь уже всё будет точно работать!</b><br>
<br>
<h2>Планы</h2><br>
...
со всеми необходимыми ключами, удалять после операции все ненужные папки. Таким образом процесс сборки PY2EXE автоматизируется до OneClick.
